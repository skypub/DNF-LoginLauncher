...
//获取游戏公告
if($temp[1]=='getnews'){
	echo str_encode($gamenews,$msgkey,$rand);
	exit;
}
...
//获取登录器值
if($temp[1]=='getdlq'){
     echo str_encode($dlqId,$msgkey,$rand);
     exit;
}
...
//获取登录器更新地址
if($temp[1]=='dlqUrl'){
     echo str_encode($dlqUrl,$msgkey,$rand);
     exit;
}
...
if($temp[1]=='getcheck'){
	if($check=='open'){
		echo str_encode("check open",$msgkey,$rand);
		exit;
	}else{
		echo str_encode("check close",$msgkey,$rand);
		exit;
   }
}
...
if($temp[1]=='getG'){
	if($G=='1'){
		echo str_encode("1",$msgkey,$rand);
		exit;
	}else{
		echo str_encode("0",$msgkey,$rand);
		exit;
   }
}
...
if($temp[1]=='getChar'){
	if($cdkex=='open'){
		...
		if(Mysql_num_rows($sql)==0){
			echo str_json_encode("getchar fail","",$msgkey,$rand);
			exit;
		}else{
			...
			$uid = $user['UID'];
			//根据UID获取角色列表
			$sql_charac = Mysql_query("select charac_no,charac_name from taiwan_cain.charac_info where m_id='$uid' and delete_flag <> 1");

			if (Mysql_num_rows($sql_charac)==0) {
				//账号下面无正常角色
				echo str_json_encode("no charac","",$msgkey,$rand);
				exit;
			}

			$characArr = array();
			while($rows = Mysql_fetch_array($sql_charac)){
				$characArr[] = $rows;
			}

			echo str_json_encode("getchar success",json_encode($characArr),$msgkey,$rand);
			exit;
		}
	}else{
		echo str_json_encode("cdk close","",$msgkey,$rand);
		exit;
	}
}
...
if($temp[1]=='cdkey'){
	if($cdkex=='open'){
		...
		//根据char_no 获取uid
		$sql_charac = Mysql_query("select charac_no,charac_name from taiwan_cain.charac_info where charac_no='$char_no'");
		...
		$user = Mysql_fetch_array($sql_charac);
		$uid = $user['UID'];
...
if($temp[1]=='login'){
    ...
    if(Mysql_num_rows($sql)==0){
		...
		echo str_encode($encrypted,$msgkey,$rand);
	      Mysql_query("update taiwan_login.member_login set m_id=$uid where m_id=$m_id");//取消异常数据
...
function str_json_encode($str,$json,$key,$key_rand){
	$arr = array('code' => $str, 'data' => $json);
	return str_encode(json_encode($arr),$key,$key_rand);
}
